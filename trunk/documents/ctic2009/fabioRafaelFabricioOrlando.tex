...
resultados. Na figura 1 é possivel
visualizar a estrutura de todos os módulos e seus componentes.  
...
%O objetivo do módulo para
%leitura do MeSH é manipular todas as
%informações contidas no MeSH. O objetivo
%do módulo para leitura dos artigos é
%identificar os descritos de cada artigo
%e o objetivo

Os componentes utilizados na
implementação dos módulos são:

\begin{itemize}
  \item \textit{Alocação do MeSH na Memória}: a leitura do XML do MeSH é feita com a API SAX (Simple API for XML) \cite{pythonsax:08}. Apesar de existirem outras API mais fáceis de usar, o SAX é o mais adequado para este trabalho, pois não preciso colocar todo o arquivo em memória para manipulá-lo.
...
Com os últimos resultados, já é possível afirmar que o corte por ocorrência é mais eficiente que o corte por quantidade, porém mais alguns testes serão feitos com ambos os cortes. Como já foi visto que o filtro nos descritores melhora a cobertura. O principal motivo é o fato de que, no conjunto de artigos utilizado, tem-se uma média de 3,19 descritores com desvio padrão de 2,17 que são definidos subjetivamente e não estão presentes no texto. O próximo resultado apresenta a combinação desta técnica mais a do bigrama na tabela 3. 
